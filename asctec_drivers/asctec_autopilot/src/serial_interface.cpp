...
namespace asctec
{
SerialInterface::SerialInterface(std::string port, uint32_t speed) :
  serialport_name_(port), serialport_speed_(speed)
{
  struct termios tio;
  status = false;
...
void SerialInterface::sendCommand(Telemetry *telemetry)
{
  ...
  if (telemetry->commandInterval_ != 0 && ((telemetry->commandCount_ - telemetry->commandOffset_)
      % telemetry->commandInterval_ == 0))
  {
