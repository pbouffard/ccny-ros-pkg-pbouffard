...
namespace asctec_autopilot
{
  SerialInterface::SerialInterface (std::string port, uint32_t speed):serialport_name_ (port), serialport_speed_ (speed)
  {
    struct termios tio;
      ...
      dev = fopen (serialport_name_.c_str (), "w+");
      ROS_ASSERT_MSG (dev != NULL, "Failed to open serial port: %s", serialport_name_.c_str ());

      ROS_ASSERT_MSG (tcgetattr (fileno (dev), &tio) == 0, "Unknown Error: %s", strerror (errno));
      ...
      ROS_ASSERT_MSG (dev != NULL, "Could not open serial port %s", serialport_name_.c_str ());
  ...
  {
    ROS_ASSERT_MSG (tcdrain (fileno (dev)) == 0, "Drain Error: %s", strerror (errno));
  }
  ...
  void SerialInterface::stall(bool wait)
  {
    ...
    ROS_ASSERT_MSG (tcsetattr (fileno (dev), TCSADRAIN, &tio) == 0, "Unknown Error: %s", strerror (errno));
  }
  ...
  bool SerialInterface::getPacket (char *spacket, unsigned char &packet_type, unsigned short &packet_crc, unsigned short &packet_size)
  {
    char stoken[4];
    char ssize[2];
    char stype[1];
    ...
    this->stall(true);
    i = fread (stoken, sizeof (char), 3, dev);
  ...
  void SerialInterface::write (char *output)
  {
    int i;
    int len = strlen (output) + 1;
    ROS_DEBUG ("Writing %d element(s): %s", len, output);
    i = fwrite (output, sizeof (char), len, dev);
    if (i != len)
    {
  ...
  bool SerialInterface::getPackets (AutoPilot *autopilot)
  {
    char cmd[16];
    char spacket[1024];
    ...
    ROS_INFO("Packet Request: %04x %d packets",(short)autopilot->requestPackets_.to_ulong(),autopilot->requestPackets_.count());
    sprintf (cmd, ">*>p%c", (short)autopilot->requestPackets_.to_ulong());
    this->write (cmd);
    this->drain();

    // try to read in packet
    bool result = false;
    for (i=0; i<autopilot->requestPackets_.count(); i++)
    {
      ...
      if (read_result)
      {
        ...
        if (packet_type == AutoPilot::PD_IMUCALCDATA)
        {
          printf ("Packet type is IMUCALCDATA\n");

          memcpy (&autopilot->IMU_CALCDATA_, spacket, packet_size);

          unsigned short checksum = crc16 (&autopilot->IMU_CALCDATA_, sizeof (packet_size));
    ...
    this->stall(false);
    i = fread (spacket, sizeof (char), 1, dev);
