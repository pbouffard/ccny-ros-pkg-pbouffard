...
struct point{
	...
	inline point(T _x, T _y):x(_x),y(_y),z(0){}
	T x, y, z;
};
...
struct orientedpoint: public point<T>{
  inline orientedpoint() : point<T>(0,0), roll(0), pitch(0), theta(0) {};
	inline orientedpoint(const point<T>& p);
	inline orientedpoint(T x, T y, A _theta): point<T>(x,y), roll(0), pitch(0), theta(_theta){}
